...
A `by decide` test that the text parser maps a header/include/comment-bearing Bell program to exactly the expected AST. From [Parse.lean](Parse.lean) (§5, lines 321–326). The whole parser is written over `List Char` (via `src.toList`) specifically so these tests stay kernel-reducible / `by decide`-checkable.
...
- **(M / deferred) No full physical correctness.** The emitted `CompiledMixIR` is checked for logical/resource legality only. QStab/QClifford lowering, syndrome extraction, decoding, and fault tolerance remain **deferred obligations** in `QASMArtifact.obligations`. [Physical.lean](Physical.lean) provides a real end-to-end path **only** for the currently verified structural fragment (logical Paulis, transversal H/S, verified transversal CNOTs, straight-line logical Pauli measurements) and **explicitly rejects** `.magic`, `.switch`, scheduled/controlled PPM, automorphisms without chunks, and parallel PPM rather than fabricating a circuit.
