...
inductive ProtocolOp
  ...
  | measureSyndrome (inputs : List ResourceId)         -- GENERIC syndrome-measure / projection primitive
...
inductive Obligation
  ...
  | syndromeDecoding    (name : String)   -- a deferred (possibly non-Pauli) syndrome decoding, e.g. RM-15 `η`
...
.qualityClaim "ε < 0.141"          -- the distillation threshold (surfaced from the protocol's output quality)
.syndromeDecoding "rm15.eta"       -- the NON-Pauli Bravyi–Kitaev A-type syndrome (η) decoding
                                   --   (emitted by measureSyndrome; the binary CSS surrogate does NOT prove it)
.syndromeDecoding "rm15.z-syndrome"

-- rejected: a non-A-type input, e.g. `.Tdg` or `.Y` (basisIncompatible).  Arity is NO LONGER a
--   primitive check — measureSyndrome is generic; "exactly 15" is the rm15_to_1 LIBRARY's property.
```
