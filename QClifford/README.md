...
| [Parse.lean](Parse.lean) | Total text parser from QClifford surface syntax (`H q1`, `CNOT q0 q1`, `Meas q3 -> c7`, …) to the `Circuit` AST; round-trip tests are `by decide`. |
...
A `CNOT(c,t)` realized from a `CZ` conjugated by Hadamards. In QClifford surface syntax
(physical qubits are bare numbers or `q<N>`; classical bits bare numbers or `c<N>`):
...
This text **parses today** to the `Circuit` AST below — `QClifford/Parse.lean`, by `decide`
(`parsesTo "H q1\nCZ q0 q1\nH q1" (cnotFromCZ 0 1) = true`):

```lean
/-- A `CNOT(c,t)` realized from a `CZ`: `H t; CZ c t; H t`. -/
def cnotFromCZ (c t : PQubit) : Circuit := [.H t, .CZ c t, .H t]

-- cnotFromCZ 0 1 is the concrete circuit value (machine form, `Circuit = List Gate`):
...
The concrete circuit value carries the resource readouts directly: 3 gates, exactly 1
two-qubit gate, width 2. Source: [Syntax.lean](Syntax.lean) (lines 90–95);
parser and round-trip test in [Parse.lean](Parse.lean) (line 112).

A larger example — standard `Z`-syndrome extraction — also parses by `decide`
(`Prep0 q3`, two `CNOT`s into the ancilla, then a `Z`-basis `Meas q3 -> c7`):
...
parses to `[.prepZero 3, .CNOT 1 3, .CNOT 0 3, .meas 3 7]` ([Parse.lean](Parse.lean), line 115).
Classical parity (`c5 := xor c0 c1`) and a conditioned Pauli (`If c2 then X q0`) parse the same way.
...
- **D (`by decide` tests).** The `cnotFromCZ` resource-counter `example`s in `Syntax.lean`, and the surface-syntax round-trip `parsesTo` `example`s in `Parse.lean` (e.g. `"H q1\nCZ q0 q1\nH q1"` → `cnotFromCZ 0 1`), are decidable checks on concrete circuits, not general lemmas.
